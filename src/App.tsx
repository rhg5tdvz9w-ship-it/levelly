...
// ─── Ref image helpers ────────────────────────────────────────────────────────
function pickRelevantRefs(vi: VisualIdentity, unitAtScene?: string, lib?: any[], scene?: string): any[] {
...
- STARTING CANNON: Unless the user-provided context explicitly states a different starting tier, the cannon at timestamp 0s is Simple Cannon. Do not call it Double, Triple, or Tank based on visual appearance alone — the single-barrel Simple Cannon is the default start for all MOC ads. A "LV+1" popup appearing early does NOT mean the cannon started at a higher tier.
...
2. +N GATE: Blue addition gate visible → description MUST say "Cannon count +[N]" (e.g. "Cannon count +1: now 2 cannons firing"). NEVER say "mob swarm passes through +1 gate" — mobs flow through the gate visually but the EFFECT is cannon count increase, not mob multiplication. The mob swarm physically passes through xN gates (mob multiply) and +N gates (cannon count up) — but describe the EFFECT correctly for each type.
   LV+1 POPUP RULE: When you see a "LV+1" or "LV+N" text animation appear on screen, this is the cannon COUNT leveling up — NOT a cannon tier upgrade. The cannon MODEL (Simple/Double/Triple/Tank) does NOT change when LV+1 appears. Do NOT write "cannon upgrades" or "cannon levels up to next tier" when you see LV+1 — write "Cannon count +1" instead.
