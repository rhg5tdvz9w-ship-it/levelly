...
export const briefSystem = (lib: any[], ctx: string, seg: string, iterateFrom?: string, refNote?: string, competitorContext?: { core_fantasy?: string; moc_inspiration?: string; transferable_elements?: string[]; title?: string; lift_intent?: string }) => {
  ...
  const escalationSignal = (highPerf.length >= 3 && withEscalation.length >= 2)
    ? `\n\nGATE ESCALATION SIGNAL (from library): ${withEscalation.length} of ${highPerf.length} winner/scalable entries use ESCALATING xN gate sequences (e.g. x2 → x6 → x60). Examples from library: ${withEscalation.slice(0, 3).map((e: any) => `"${e.title || e.creative_id}" → ${e.gate_escalation}`).join("; ")}. When proposing concepts, consider xN escalation as a HIGH-VALUE MECHANIC for 1-2 of the concepts. Mark those as is_experimental: false (escalation is proven). In visual_identity.gate_values, list the full escalation sequence (["x2","x6","x60"]). In lane_design, describe how the escalating gates are spatially arranged. In tension_moments, anchor the peak to the final xN gate pass where the mob count explodes.`
    : "";
