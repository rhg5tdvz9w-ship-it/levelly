...
Return ONLY JSON: {"hook_timing_seconds":number,"hook_type":"Challenge|Satisfying|Loss Aversion|Story|FOMO|Tutorial","hook_format":"in-game|live-action","hook_description":string}

hook_format: "live-action" if the opening shot shows a real human being (UGC archetype — TikTok-native shot of a real person reacting/talking, photo-realistic). "in-game" if the opening is the game's stylized art (cartoon 3D, gameplay scene, boss reveal, lane).`;
...
RULE 5 — CHAMPION PRESENCE EVIDENCE (Deploy BB — fixes name-from-references contamination):
Every entry in champions_visible REQUIRES explicit frame evidence. A character must appear in AT LEAST 2 frames with at least ONE of these visible:
(a) HP bar rendered ON the character with character-specific name/sprite
(b) Characteristic silhouette/colour match against a MOC reference asset (e.g. mobzilla = green T-Rex with pink spines and red mouth, hulk_champion = large blue muscular humanoid blonde hair, yellow_normie = yellow-skinned humanoid round bald head — name alone is insufficient, you must SEE the matching body shape)
(c) Character-specific animation or asset clearly distinguishable from generic enemy mobs

If a candidate champion does NOT meet (a/b/c) across 2 frames → put them in champions_unverified[] instead of champions_visible[]. champions_unverified is for producer review — do NOT treat as confirmed presence in DNA fields (key_mechanic, why_it_works, etc).

CONTAMINATION GUARD: MOC VISUAL REFERENCES (provided in this prompt) are reference IMAGES of characters that EXIST in the MOC game. They are NOT a list of characters present in THIS specific video. The fact that a reference image is provided does NOT mean that champion appears in this ad. champions_visible is for what you CAN POINT TO IN A FRAME. If you cannot point to a specific frame where the champion is unambiguously visible — do not list them as visible.

When in doubt → champions_unverified, not champions_visible. Empty champions_visible is acceptable for ads with no clear champion (e.g. only generic mob swarm + tower).
...
MOC STRATEGIC LAYER (BC2.4 — mandatory for every MOC entry. Applies to winners AND failures so we can learn from both — failure pattern data is as valuable as winner pattern data):
- core_fantasy_moc: 3-8 word fantasy this MOC ad sells. Examples: "near-loss reversal", "delayed power compounding", "incomplete-collection anxiety", "swarm dominance triumph", "gate-choice regret". Should reflect the EMOTIONAL CORE of why a Whale (45-59M USA, completionist/relax) or Dolphin (35-44M USA, competitive/completionist) would watch.
- winning_pattern: 1-2 sentences naming WHY THIS ad worked (if tier=winner/scalable) OR WHY IT FAILED (if tier=failed). Cite specific frame evidence — what mechanic, what moment, what visual. NOT a platitude. NOT "strong hook, clear mechanic" — be specific to this entry. For failed entries, pinpoint the failure beat (e.g. "Mob count drops to 1 at 18s before any payoff — viewer abandons before reward hits").
- replicable_elements: array of 3-5 testable elements from this entry that could anchor next briefs. ACTIONABLE format only — "<concrete element from this ad> → testable hypothesis for next brief". Bad: "good pacing". Good: "F2/V1+V5 wooden barrier with +99 gates revealed behind, breaks at 6s → test this same break-then-reveal architecture with metal/concrete obstacle in next brief". For failed entries, replicable_elements should describe what to AVOID (e.g. "x100 gate at 4s spikes mob count too early → test delayed escalation, x10 at 6s instead").`:`${UNIVERSAL_EVENTS_GUIDE}`}
${config.ad_type==="compound"?"COMPOUND: is_compound:true, segments array required.":""}

IS_COMPOUND CONSERVATIVE RULE (Deploy S): set is_compound:true ONLY if there is CLEAR scene-stitching evidence — multiple distinct ads back-to-back with hard cuts AND completely different gameplay/UI/biome between segments. Normal scene cuts within a single ad (e.g. cuts between gameplay angles, fade transitions, hook then gameplay) are NOT compound. Default is_compound:false unless you can cite specific timestamps where one ad obviously ends and a different one begins. False compound flags are noisy and worse than missed ones.
${config.ad_type==="competitor"?"COMPETITOR MODE: Strip ALL MOC-specific vocabulary (Yellow Normie, Tank/Simple/Double/Triple Cannon, named Mob Control champions, MOC-specific biomes). Describe units generically: small unit, large unit, boss 1, boss 2. Use universal event tags only (hook, gate, power_up, obstacle, peak, etc). REQUIRED competitor-only fields — fill all FIVE precisely: (1) core_fantasy: the emotional or mechanical fantasy this ad sells in 3-8 words (e.g. idle reward compounding, delayed gratification payoff, progress wipeout anxiety). (2) transferable_elements: array of 3-5 concrete testable hypotheses for MOC — each must be ACTIONABLE (what to change/add in MOC and what it tests). Bad example: use bigger numbers. Good example: Escalating +N gate values (5 → 50 → 500 → 5000) instead of static +1/+3 — tests whether numerical progression itself drives engagement independent of power gain. (3) moc_inspiration: 1-2 sentence high-level summary of how this ad maps to MOC strategy. (4) moc_lift_concrete (BC2.4): ONE specific scene/mechanic from this ad to recreate in MOC, with timestamp + concrete MOC translation. Format: '[Ts] <competitor mechanic in plain words> → MOC: <concrete translation in MOC vocabulary (lane/cannon/gate/boss)>'. Example: '[6s] Gold and Goblins shows passive coin auto-collection at the base → MOC: opening 4s shows mob swarm passively collecting +1 gate panels in a parked-cannon scene before any threat appears.' Replaces the vague moc_inspiration as the producer-actionable field. (5) game_title: the NAME OF THE GAME/APP (NOT a description of the ad content). Extract from visible branding — app logo, game name in splash, watermark, UI text. Examples: 'Last War', 'Gold & Goblins', 'Whiteout Survival', 'Kingshot'. If the game_title is PROVIDED in upload_context by the user, use that exact value. If unclear and no user context, return null — do not invent.":""}
hook_format detection (Deploy Y2 → BC2 P6 strengthened): SCAN ALL FRAMES (not just opening) — if ANY frame in the first 8 seconds shows a real human via photo-realistic capture, classify the ad as "live-action".

DETECTION CRITERIA — classify as "live-action" if ANY of:
(a) Photographic skin tone, real face proportions, lighting that looks like camera capture (not 3D render)
(b) Real human hand visible holding a phone, controller, or product — NOT a 3D-rendered character holding things
(c) Indoor home setting (couch, kitchen, bedroom, office) with natural lighting — NOT a stylized game environment
(d) UGC framing patterns: vertical 9:16 with face-cam, split-screen with reaction, jump cuts between human + gameplay
(e) Voice-over implied (text on screen reading like spoken caption, e.g. "POV:", "wait until you see this", "I tried this game")

CLASSIFY AS "in-game" ONLY IF:
- All frames are 3D-rendered or 2D animated game art (cartoon stylization, low-poly, MOC aesthetic)
- No real human visible at any point in the ad
- Setting is fictional (lane, base, dungeon, fantasy environment) — not real-world

EDGE CASES:
- Game gameplay framed inside a phone-cam shot of a person holding the phone → "live-action" (the human is the hook)
- Picture-in-picture with face cam corner + gameplay main → "live-action"
- Realistic 3D character that looks human-like but is clearly a game character (no skin texture, no real lighting) → "in-game"
- Photo-real opening that cuts to gameplay at 4s → "live-action" (the OPENING was the hook)
...
{"key_events":[{"timestamp_seconds":number,"event_type":"hook|gate|power_up|boss_appear|boss_damage|boss_death|obstacle|peak|almost_fail|almost_win|loss","description":string,"frame_evidence":string}],"title":string,"is_compound":boolean,"transition_type":string|null,"segments":[]|null,"hook_type":"Challenge|Satisfying|Loss Aversion|Story|FOMO|Tutorial","hook_format":"in-game|live-action","hook_timing_seconds":number,"hook_description":string,"gate_sequence":[string],"gate_escalation":string|null,"swarm_peak_moment_seconds":number|null,"loss_event_type":"Wrong Gate|Boss Overwhelm|Obstacle Hit|Death Gate|Enemy Overwhelm|None","loss_event_timing_seconds":number|null,"unit_evolution_chain":[string],"cannon_count_log":string,"emotional_arc":string,"biome":"Desert|Cyber-City|Forest|Volcanic|Snow|Toxic|Water|Bunker|Meadow|Unknown","biome_visual_notes":string,"champions_visible":[string],"champions_unverified":[string],"giant_kills":[{"timestamp_seconds":number,"giant_name":string,"note":string}],"pacing":"Fast|Medium|Slow","key_mechanic":string,"mechanic_family":"F1|F2/V1|F2/V2|F2/V3|F2/V4|F2/V5|F2/V1+V5|F2/V3+V5|F2/V4+V5|F2/V1+V3|F3|F4|F3+F2/V1|F2/V4+F4|F4+F2/V3|other","why_it_works":string,"creative_gaps_structured":{"hook_strength":string,"mechanic_clarity":string,"emotional_payoff":string,"tension_arc":string,"rewatch_factor":string},"frame_extraction_gaps":string${config.ad_type==="moc"?",\"core_fantasy_moc\":string,\"winning_pattern\":string,\"replicable_elements\":[string]":""}${config.ad_type==="competitor"?",\"moc_inspiration\":string,\"moc_lift_concrete\":string,\"core_fantasy\":string,\"transferable_elements\":[string],\"game_title\":string|null":""}}`;
...
export const briefSystem = (lib: any[], ctx: string, seg: string, iterateFrom?: string, refNote?: string, competitorContext?: { core_fantasy?: string; moc_inspiration?: string; transferable_elements?: string[]; title?: string; lift_intent?: string; gate_sequence?: string[]; unit_evolution_chain?: string[]; key_mechanic?: string; pacing?: string; tension_moments?: string[]; biome?: string; hook_type?: string; hook_description?: string; gate_escalation?: string | null; user_visual_reference?: any; user_visual_references?: Array<{ name?: string; description?: any }> }, marketIntel?: any) => {
  ...
  // Deploy BC2 P3+P5: handle multi-image-ref array. Falls back to single user_visual_reference for compat.
  // KEY CHANGE (Deploy BC2 P5): when user provides image refs WITH lift_intent, ALL 4 concepts must
  // respect the visual reference (not just "at least 1"). Lift intent OVERRIDES the outsider concept-4
  // anchor rule — user explicit intent wins.
  const visualRefs: Array<{ name?: string; description?: any }> = Array.isArray(competitorContext?.user_visual_references) && competitorContext!.user_visual_references!.length > 0
    ? competitorContext!.user_visual_references!
    : (competitorContext?.user_visual_reference ? [{ name: "user_ref", description: competitorContext.user_visual_reference }] : []);
  ...
  const userImageRefBlock = visualRefs.length > 0 ? `\n\nUSER VISUAL REFERENCES (${visualRefs.length} uploaded image${visualRefs.length > 1 ? "s" : ""}, extracted by Gemini Vision — describes what is IN each image in its OWN terms, not MOC vocabulary):\n${visualRefs.map((r, i) => `\n[REF ${i + 1}${r.name ? ` — ${r.name}` : ""}]:\n${typeof r.description === "string" ? r.description : JSON.stringify(r.description, null, 2)}`).join("\n")}\n\nFIRST-PASS REF EXTRACTION (BC2.5.1 — STRUCTURED extraction, do this internally before writing concepts):\nFor EACH visual reference above, extract these 8 fields internally (do NOT surface in JSON output, but USE them to drive lane_design / gate_values / biome / hook_a/c):\n  - lane_topology: ONE of: "single" (one straight central lane), "fork" (2-3 parallel sub-lanes player picks between), "winding" (single lane that curves/spirals), "base" (no lane, stationary base layout). Pick the closest match — do NOT default to "fork" unless the ref clearly shows 2+ parallel paths.\n  - vertical_stack: array of elements visible on the central lane bottom-to-top in order (e.g. ["cannon", "+1 gate", "x2 gate", "x3 gate", "x4 gate", "boss"]). This is the player's progression sequence.\n  - left_side_elements: array of what sits to the LEFT of the central lane (e.g. ["+1 gate"], ["empty desert"], ["winding enemy mob path"], ["sub-lane with +1 stack"]).\n  - right_side_elements: array of what sits to the RIGHT (e.g. ["upgrade container with cannon-tier visible inside"], ["sub-lane with xN gates"], ["empty"]).\n  - enemy_arrangement: ONE of: "linear" (mobs on the lane), "winding" (mob path curves/spirals around the lane), "perimeter" (mobs surround), "wave" (mass advancing), "absent" (no enemy visible mid-lane).\n  - upgrade_payoff_visible: yes/no — is there a visible cannon-tier upgrade element (golden jet, tank, triple cannon visible inside or on top of a container)? If yes, brief description.\n  - obstacle_types: array of obstacle/mechanic words visible (gate, container, barrier, lift platform, breakable wall, none, etc).\n  - biome_hint: which MOC biome best matches the visual (Desert, Foggy Forest, Water, Bunker, Volcanic, Snow, Cyber-City, Toxic, Meadow, none).\n\nARCHITECTURAL LIFT RULE (BC2.5.1 — when ref shows specific level-design patterns, EVERY concept must include those patterns in its lane_design. KEEP family diversity — this rule operates INDEPENDENTLY of the FAMILY pre-flight check):\nIF lift_intent mentions any of: "lane", "layout", "level design", "structure", "level", "topology", "sub-lane", "level layout" — OR if visual_refs are present — THEN every concept's lane_design / gate_values / hook_a_description MUST include the ref's specific architectural signatures, wrapped in that concept's own family/biome/boss context:\n\n  1. VERTICAL_STACK PROGRESSION — if vertical_stack shows escalating multipliers (x2→x3→x4) or escalating accumulators (+1→+5→+99), every concept's gate_values array MUST reflect a similar escalation pattern. EXAMPLES: F1 concept shows the escalation as a single straight column. F2/V1 concept shows the escalation revealed AFTER breaking a barrier. F3 concept places the escalation on ONE of the sub-lanes (the other lane is the diverging choice). F4 concept maps escalation to base-resource counters ramping. Same escalation signature, four different family wrappers.\n\n  2. UPGRADE PAYOFF VISUAL — if upgrade_payoff_visible is "yes" (e.g. golden jet / tank / triple cannon visible inside a container), EVERY concept's lane_design MUST describe a visible upgrade container with the upgrade tier rendered inside it. Translate the visual into MOC vocab. EXAMPLE: ref shows "blue container HP:20 with golden jet visible inside" → concept lane_design must describe "Right side: blue/wooden/stone container HP:20 with [Triple Cannon | Tank | Golden Jet] visible inside, signaling the upgrade payoff." This is non-negotiable; producer-side, the upgrade-with-cannon-visible is the strongest install hook.\n\n  3. ENEMY_ARRANGEMENT — if enemy_arrangement is "winding" (mob path curves/spirals around the lane) or "perimeter" (mobs surround the lane), describe that exact arrangement in EVERY concept's hook_a_description AND lane_design. The default "linear mid-lane red mobs" placement is FORBIDDEN when the ref shows a non-linear arrangement. EXAMPLE: ref shows winding red mob river → every concept opens with "winding red mob river snaking around the lane toward boss" not "red mobs in the lane."\n\n  4. SIDE ELEMENTS — translate left_side_elements and right_side_elements into MOC vocab and place them on the corresponding side in every concept's lane_design. If left_side_elements = ["+1 gate stack"], every concept has a +1 gate stack on the left. If right_side_elements = ["upgrade container with cannon-tier inside"], every concept has it on the right. The CONTEXT around the elements may vary (biome, boss, family wrapper) but the elements themselves are constant across all 4.\n\nFAMILY DIVERSITY IS PRESERVED — concepts may still span different families per the pre-flight check. The architectural signatures are LIFTED CONTENT that lives WITHIN each family's wrapper. A producer comparing the ref to the 4 generated renders should see the SAME architectural elements (escalating stack, upgrade payoff, enemy arrangement, side elements) but rendered in 4 different family/biome/boss contexts.\n\nVISUAL REFERENCE APPLICATION RULES:\n${hasLiftIntent
  ...
  // BC2.3: 5-field render propagation rule — fires on ANY concept with intel_source set, not just concept 4 [OUTSIDER:].
  // Closes audit gap where transferable_elements lived in narrative copy but never reached lane_design / gate_values / hook_a / hook_c.
  const hasAnyIntel = !!competitorContext || (visualRefs.length > 0) || !!(marketIntel as any)?.digest;
  const intelPropagationBlock = hasAnyIntel ? `\n\nINTEL PROPAGATION TO RENDER FIELDS — applies to EVERY concept where intel_source is set (not null):\nThe lifted element MUST appear in ALL FIVE of these render-visible fields. Narrative-only mention in experimental_note is INSUFFICIENT.\n  1. concept.lane_design — the spatial layout MUST reflect the intel's mechanic. Describe the lane in MOC-native terms (gates, containers, sub-lanes) showing the lifted mechanic in physical space.\n  2. concept.production_script — at least 2 of the timeline beats must explicitly demonstrate the lifted mechanic in action (the auto-collection ticks, the obstacle being broken, the gate counter ramping, the platform lifting, etc).\n  3. concept.hook_a_description — Hook A composition must include the lifted mechanic prominently. For F1 baseline concepts: boss-dwarfs-cannon. For intel-using concepts: boss + lifted mechanic both visible from the FIRST FRAME.\n  4. concept.hook_c_description — Hook C must demonstrate the lifted mechanic's signature moment (the high-tension beat that makes the lift visible).\n  5. concept.visual_identity.gate_values — if the intel changes the gate vocabulary (e.g. resource counters instead of +N, healing pads instead of multipliers), reflect that in gate_values. Default ["+1","+1","x2"] is F1 baseline only — intel-using concepts may use ["+99","+99","x100"] or ["+99 RESOURCE","+99 RESOURCE","x2 HEAL"] etc.\n\nThe intent: a producer looking at the 4 scene renders should see STRUCTURAL DIFFERENCES between intel-using concepts and baseline. If concept 2 is "Multi-Boss Sequential Challenge" but renders look identical to concept 1 with one extra boss icon, the lift failed. Concept 4 with [OUTSIDER:] tag still has its own additional propagation rules in the OUTSIDER ANCHOR block — those are stricter; this block is the floor for all intel-using concepts.` : "";
  const competitorBlock = competitorContext ? `\nCOMPETITOR INSPIRATION — the user uploaded a competitor ad and wants you to lift specific elements:\n- Source: ${competitorContext.title || "competitor ad"}\n- Core fantasy: ${competitorContext.core_fantasy || "unspecified"}\n- High-level MOC mapping: ${competitorContext.moc_inspiration || "unspecified"}${expandedDnaBlock}\n- Transferable elements: ${(competitorContext.transferable_elements || []).map((e, i) => `\n  ${i+1}. ${e}`).join("")}\n${competitorContext.lift_intent ? `- USER LIFT INTENT (PRIMARY SIGNAL — lift THIS specifically): ${competitorContext.lift_intent}\n` : ""}\nAPPLICATION RULES:\n1. Concept 1 stays pure MOC-native (proven baseline, no competitor lift) UNLESS user provided lift_intent — in that case Concept 1 still favors proven biomes but its render fields must reflect the lift (see USER VISUAL REFERENCES block above and PRECEDENCE rule). Concepts 2, 3, 4 must EACH lift from a different intel source — different competitor entries OR different differentiation_axes from the synthesis OR different transferable_elements OR (if visual refs present) different refs. No two concepts may share the same source.

1b. OUTSIDER ANCHOR RULE (Deploy Y, strengthened in Z4):
   When intel above contains GENRE OUTSIDERS (1+ entries):
     CONCEPT 4 IS THE OUTSIDER CONCEPT. Its entire mechanic backbone IS the outsider's moc_translation.
     Pick ONE outsider. Build concept 4 around its moc_translation.
     Concept 4's experimental_note MUST start with "[OUTSIDER:<outsider_title>] " followed by [FAMILY:...] tag.
     Concept 4 cannot be a wild-card invention or a competitor lift — it MUST be the outsider.
     Family/variant for concept 4 may combine with the outsider's translation. Tag example: "[OUTSIDER:Gold and Goblins Ad] [FAMILY:F4+F2/V1]".
   When intel has NO genre_outsiders (empty list), concept 4 reverts to wild-card invention with [FAMILY:...] tag only.
   When intel has 2+ outsiders, producer choice is yours but cite which one in the tag.

   OUTSIDER PROPAGATION (Deploy Z4 — outsider mechanic must reach EVERY render-visible field, not just narrative):
   When concept 4 has [OUTSIDER:...] tag, the outsider's mechanic MUST propagate to ALL of:
     1. concept.lane_design — the spatial layout MUST reflect the outsider's mechanic. If outsider is "idle resource collection" (Gold and Goblins), the lane shows resource nodes/auto-collectors prominently — not just standard +N/xN gates with one outsider element bolted on. If outsider is "support/healing meta-game" (Last Asylum), the lane shows healing/buff stations or a meta-base that mobs interact with.
     2. concept.production_script — at least 3 of the timeline beats must explicitly demonstrate the outsider's mechanic in action (the auto-collection ticks, the healing buff applying, the idle counter ramping, etc).
     3. concept.hook_a_description — Hook A (gameplay boss) composition must INCLUDE the outsider element prominently. Default is boss-dwarfs-cannon, but for outsider concepts: boss + outsider mechanic both visible. Example: instead of "Red Hulk fills 65% of frame, cannon tiny", show "Red Hulk in background + auto-collecting resource nodes glowing across the lane + cannon at bottom" so the outsider mechanic is established from the FIRST FRAME.
     4. concept.hook_c_description — Hook C must demonstrate the outsider's signature moment. For idle outsiders: a counter ramping up. For management outsiders: a meta-game element being interacted with. Hook C is where the outsider's flavor SELLS the disruption.
     5. concept.visual_identity.gate_values + cannon_type — if outsider replaces standard gates with something else (resource counters, healing pads), reflect that in gate_values array. Example: ["+99 RESOURCE", "+99 RESOURCE", "x2 HEAL"] not ["+1", "+1", "x2"].
   The intent: brief 4 must look STRUCTURALLY DIFFERENT from briefs 1-3 in its renders, not just have a different word in the experimental_note. If a producer looks at the 4 scene renders and brief 4 looks like brief 2 with a slight color tweak, the outsider rule failed.

   OUTSIDER LIFT QUALITY GUARDRAILS (Deploy BB — prevents unshippable creative-writing outsider concepts):

   GUARDRAIL 1 — BIOME-MECHANIC SEPARATION. The outsider concept's biome MUST NOT visually match the outsider's source genre. Aesthetic match disguises mechanical absence. Forbidden pairings:
     - Whiteout Survival outsider → forbidden in Snow biome (force Desert / Volcanic / Cyber-City / Meadow / Bunker / Toxic / Foggy Forest)
     - Last Asylum outsider → forbidden in Cyber-City / Volcanic / Toxic / Bunker (any wasteland-coded biome) — force Desert / Meadow / Water / Snow
     - Lords Mobile outsider → forbidden in Meadow (medieval-coded) — force Volcanic / Toxic / Cyber-City / Bunker
     - Gold and Goblins outsider → forbidden in Bunker / Volcanic (cave-coded) — force Desert / Meadow / Snow / Foggy Forest
     - General rule: pick the outsider biome to MAXIMIZE visual contrast with the source. This forces the lift to be MECHANICAL not AESTHETIC.

   GUARDRAIL 2 — MOC VOCABULARY LOCK. Outsider concept's lane_design, production_script, hook_a_description, hook_c_description, and visual_identity.gate_values may use ONLY MOC-native terms:
     ALLOWED: gate, container, +N, xN, mob swarm, cannon tier (Simple/Double/Triple/Tank), boss HP, biome name, upgrade obstacle, +1, +5, x3, x100, mob count, near-win, almost-fail, cannon count, lane, road, sub-lane.
     FORBIDDEN net-new entity types: "freeze aura", "shield pulse", "heal beacon", "buff station", "mining drill", "fire trail", "passive debuff", "AOE effect", "lift platform" (unless tagged F2/V4 family), "auto-collector" (unless explicitly inside F4 base layout), or any other novel system that doesn't exist in MOC's mechanic vocabulary.
     If the outsider's mechanic CANNOT be expressed with allowed terms only — pick a different outsider, OR drop to F2 variant translation, OR translate as numerical-progression escalation (highest-fidelity lift in MOC vocab).

   GUARDRAIL 3 — SINGLE-ELEMENT GRAFT. The outsider's moc_translation = ONE mechanical idea expressed in MOC vocabulary. NOT a feature port. NOT 2+ stacked novel mechanics.
     GOOD lift: "Whiteout's environmental-destructibles → MOC F2/V1 ice block container with cannon icon, breaking it reveals +N gates behind." (one mechanic: break-to-unlock)
     BAD lift: "Whiteout's environmental-destructibles + idle freeze-aura buff system → ice blocks that reveal gates AND apply slow debuff to enemy mobs." (two novel mechanics stacked, second one violates Guardrail 2)
     If translation contains 2+ novel mechanics → keep ONE, drop the rest. Pick the one most expressible in MOC vocabulary.

   GUARDRAIL 4 — PRODUCIBILITY CHECK. Concept must describe how a producer shoots this with existing MOC assets + AT MOST 1 new asset/effect modification. Surface as producibility_check field:
     "pass" — concept can be shot with existing MOC art assets (cannon, mob, gate, container, biome) + maybe ONE new visual asset (e.g. a new container shape, a new biome backdrop). Producer can ship in 1 production cycle.
     "fail" — requires 3+ new assets/effects/systems. Not shippable. REGENERATE this concept.
   If producibility_check is "fail" → regenerate concept 4 with simpler outsider lift, or pick a different outsider, or fall back to wild-card invention. NEVER ship a "fail".\n2. For each competitor-inspired concept: is_experimental:true, experimental_note explains which element was lifted and how it translates to MOC mechanics.\n3. CRITICAL — lift intent must flow to render-visible fields, not just narrative text:\n   - If user lift intent mentions "lane" / "level design" / "layout" / "structure" → TRANSCRIBE the competitor s lane structure directly into concept.lane_design (describe the lane layout in concrete MOC terms: where +N gates sit, where xN gates sit, where upgrade obstacles sit, what creates the tension).\n   - If lift intent mentions "biome" / "environment" / "visuals" / "art" → set concept.visual_identity.environment to reflect competitor s biome (translated into a MOC-compatible biome name — e.g. competitor s "crystal cavern" → MOC "Cyber-City" or "Toxic").\n   - If lift intent mentions "gates" / "multipliers" / "numbers" / "escalation" → transcribe competitor s numerical progression into concept.visual_identity.gate_values array.\n   - If lift intent mentions "hook" / "opening" / "start" → mirror competitor s hook mechanic in concept.hook_description and hook_a_description.\n4. NEVER copy competitor VISUALS directly (no competitor art style, no competitor characters). Translate the MECHANIC only, in MOC vocabulary.\n` : "";
...
LIBRARY USAGE (BC2.4):
- For DATA-BACKED concepts (1-2): pull from the \`replicable_elements\` arrays of winner/scalable entries above. When a concept lifts a replicable_element, set intel_source = "moc_winner:<title>" and propagate to render fields per INTEL PROPAGATION block.
- For FAILED entries (tier=failed): do NOT replicate the elements — but their \`winning_pattern\` field describes WHAT WENT WRONG. Use this as a "what to avoid" filter when picking replicable_elements from winners. If a winning concept and a failed concept share a mechanic but differ on one execution detail, that detail is the lift signal.
- Reference \`core_fantasy_moc\` to align concepts emotionally with what Whale/Dolphin actually responds to in MOC winners.

BRIEF: ${ctx} | SEGMENT: ${seg}
AUDIENCE PROFILE:
${seg==="Whale"?"Whale: Age 45-59 (68%), male, USA. Completionist mindset — #1 motivation is unlocking all elements. Opens game to RELAX (46%). 217 avg active days. Almost-win hook = incompleteness anxiety (so close to unlocking), not competitive pressure.":"Dolphin: Age 35-44, male, USA. Mix of completionist + competitive. 170 avg active days. Responds to clear progression path and almost-win tension."}${refBlock}${visualRefBlock}${competitorBlock}${userImageRefBlock}${intelPropagationBlock}
...
MECHANIC MIXING PATTERNS (Deploy X — combinations are encouraged, here's how to think about them):
Mixing across families produces fresh mechanics that pure single-family concepts cannot. Use for at least 1-2 of concepts 2-4.

Example combinations:
  MIX 1 — F3 + F2/V1: multi-lane choice WHERE one lane has a breakable barrier hiding bigger reward.
    Visual: 3-lane fork, left = safe +1 gates, center = wooden wall blocking +99 gates, right = upgrade station. Player must commit cannons toward center to break the wall, sacrificing safe progression.
  MIX 2 — F2/V4 + F4: lift platform that holds aloft a base/upgrade station from F4 idle.
    Visual: cannon shoots up to lift a platform; while lifted, a small base on the platform auto-collects resources; resources stream down as +N gates. Stop shooting, platform falls, gates stop.
  MIX 3 — F2/V1+V5 + F3: dual-lane where each lane has destructible obstacles, but obstacle types differ.
    Visual: left lane = swarm-destroy ice block (mobs swarm), right lane = cannon-shoot wooden barrier (V1). Player picks interaction style.
  MIX 4 — F4 + F2/V3: stationary base with a pushable obstacle at front.
    Visual: top-down base view, mobs gather resources to a center upgrade core, but a pushable boulder blocks the base entrance — mobs must rotate it to clear path. Combines idle progression with active interaction.

When mixing, the experimental_note tag uses + format: [FAMILY:F3+F2/V1] or [FAMILY:F2/V4+F4] etc. Mixed concepts MUST have lane_design that describes BOTH mechanics' contribution to the layout.

PRE-FLIGHT DIVERSITY CHECK (Deploy X — HARD CONSTRAINT, run BEFORE writing concepts):
Before writing any concept body, do this internally:
  STEP 1. Decide a family/variant tag for each of the 4 concepts. Format: [FAMILY:F1] or [FAMILY:F2/V1] or [FAMILY:F2/V3+V5] or [FAMILY:F3+F2/V1] (mixed) etc.
  STEP 2. Verify ALL 4 tags are DISTINCT. Two concepts cannot share an identical tag. F2/V1 and F2/V1+V5 ARE different (combination differs). F2/V1+V5 and F2/V5+V1 are the SAME (combinations are unordered).
  STEP 3. Verify the 4 tags collectively span at least 3 different FAMILIES (F1/F2/F3/F4). Cannot be 4× F2 with different variants. Acceptable: F1, F2/V1, F3, F4 (4 families). Acceptable: F1, F2/V1, F2/V3+V5, F3 (3 families × different variant configs). Unacceptable: F2/V1, F2/V2, F2/V3, F2/V4 (only 1 family).
  STEP 4. If any check fails, change one or two concepts' tags BEFORE writing concept bodies.

Concept 1: SOFT default to F1 (proven baseline). May deviate if competitor intel strongly justifies a different family.
Concepts 2, 3, 4: each MUST have a tag distinct from concept 1 AND distinct from each other. Mechanic mixing (e.g. F3+F2/V1 multi-lane with breakable obstacles in one lane) is encouraged for 1-2 of them.

Each concept's experimental_note MUST start with the structured family tag in square brackets, then the explanation. Format:
...
hook_b_description: UGC HOOK (TikTok-native photo-realistic shot of a real person, NOT MOC art style).
SOURCING PRIORITY (Deploy X — intel-first, menu fallback):
  PRIORITY 1: If competitor intel above includes UGC HOOK PATTERNS with 2+ entries, concepts 2, 3, 4 MUST EACH lift from a DIFFERENT entry in that intel list. Concept 1 may use the menu freely.
  PRIORITY 2: When lifting from intel, transcribe the specific UGC mechanic (the action/persona archetype the intel pattern names) into a concrete photo-realistic shot description. Cite the intel source in experimental_note as "hook_b lifted from intel: <pattern name>".
  PRIORITY 3 (fallback only): If intel UGC patterns are absent or have <2 entries, fall back to the randomized menu below.

Two RANDOMIZED shortlists per generation — assign one archetype + one persona to each concept, all 4 different:
...
hook_c_description: INTEL-DRIVEN HOOK (Deploy V — must lift from competitor library or UGC hook patterns; NOT a generic stopwatch template).
APPLIES TO: concepts 2, 3, 4 (concept 1 may use a generic stopwatch hook for safety baseline).
SOURCING PRIORITY (in order):
  1. If competitor intel block is present above, lift the SPECIFIC HOOK MECHANIC from one of the competitor entries (different competitor for each concept — no two concepts may lift from the same competitor's hook).
  2. If UGC HOOK PATTERNS block is present, lift one pattern (different pattern per concept).
  3. Translate the lifted mechanic into 9:16 MOC cartoon 3D art style — character/visual translates, the MECHANIC stays.
  Examples of lifts: countdown timer overlay → MOC cannon with floating numerical timer; viral "you won't believe" reveal → MOC cannon dwarfed by surprise giant boss reveal; "I tried X" archetype → MOC cannon facing 3 lane options with one obviously trapped.
For each concept's hook_c_description, the experimental_note must include "hook_c lifted from: <competitor title or UGC pattern label>" so the producer knows the source.
