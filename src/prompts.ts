...
export const briefSystem = (lib: any[], ctx: string, seg: string, iterateFrom?: string, refNote?: string, competitorContext?: { core_fantasy?: string; moc_inspiration?: string; transferable_elements?: string[]; title?: string; lift_intent?: string; gate_sequence?: string[]; unit_evolution_chain?: string[]; key_mechanic?: string; pacing?: string; tension_moments?: string[]; biome?: string; hook_type?: string; hook_description?: string; gate_escalation?: string | null; user_visual_reference?: any; user_visual_references?: Array<{ name?: string; description?: any }> }, marketIntel?: any) => {
  ...
  const userImageRefBlock = visualRefs.length > 0 ? `\n\nUSER VISUAL REFERENCES (${visualRefs.length} uploaded image${visualRefs.length > 1 ? "s" : ""}, extracted by Gemini Vision — describes what is IN each image in its OWN terms, not MOC vocabulary):\n${visualRefs.map((r, i) => `\n[REF ${i + 1}${r.name ? ` — ${r.name}` : ""}]:\n${typeof r.description === "string" ? r.description : JSON.stringify(r.description, null, 2)}`).join("\n")}\n\nVISUAL REFERENCE APPLICATION RULES:\n${hasLiftIntent
    ? `- USER PROVIDED EXPLICIT LIFT INTENT (see USER LIFT INTENT below) — ALL 4 concepts MUST respect the visual reference(s). The lift_intent describes HOW to use the references. This OVERRIDES the OUTSIDER ANCHOR rule for concept 4 — when explicit lift intent is set, concept 4 must lift from the user's visual reference, NOT from a genre outsider.\n- Across ALL 4 concepts: the user's visual reference structure (lane layout, obstacle types, mechanics shown) MUST appear in concept.lane_design + concept.production_script + concept.hook_a_description + concept.visual_identity.gate_values. Not optional.\n- If multiple refs are provided (${visualRefs.length} here), SYNTHESIZE — combine elements from different refs into each concept (e.g. concept 1 takes lane layout from ref 1 + biome from ref 2; concept 2 takes obstacle interaction from ref 3 + gate values from ref 1; etc). No concept may rely on only one ref.\n- Translate visuals to MOC vocabulary (cannon, mob swarm, +N/xN gates, container, biome). Forbid net-new entity types not in MOC vocab.`
    : `- At least 1 of concepts 2-4 should map a reference into MOC vocabulary. Use "moc_translation_note" inside each reference if present.\n- If multiple refs (${visualRefs.length} here), distribute across different concepts — don't have all concepts lift from the same ref.\n- If a reference is non-applicable (UI screenshot, abstract, no game elements), the moc_translation_note will say so — treat as inspirational mood only.`}` : "";
